...
export default function Authentication() {
  ...
  //            state: 쿠키 상태            //
  const [cockies, setCookie] = useCookies();
  ...
  const SignUpCard = () => {
    ...
    //            state: 패스워드 에러 상태           //
    const[isPassowrdError, setPasswordError] = useState<boolean>(false);
    //            state: 패스워드 확인 에러 상태           //
    ...
    const signUpResponse = (responseBody: SignUpResponseDto | ResponseDto | null) => {
      ...
      if(code === 'DT') {
        ...
        setTelNumberErrorMessage('중복된 휴대폰 번호입니다.');
      }
      if(code === 'VF') {
        alert('모든 값을 입력해주세요');
      }
    ...
    const onNextButtonClickHandler = () => {
      ...
      if(!isEmailPattern) {
        setEmailError(true);
        setEmailErrorMessage('이메일 주소 형식이 맞지않습니다.')
      } else {
        setEmailError(false);
        setEmailErrorMessage('');
      }
    ...
    const onSignUpButtonClickHandler = () => {
      ...
      if(!isEmailPattern || !isEqualPassword) {
        setPage(1);
        return;
      }
      ...
      <div className='auth-card'>
        <div className='auth-card-box'>
          <div className='auth-card-top'>
            ...
            {page === 1 && (
              <>
                <InputBox ref={emailRef} label='이메일 주소*' type='text' placeholder='이메일 주소를 입력하세요.' value={email} onChange={onEmailChangeHandler} error={isEmailError} message={emailErrorMessage} onKeyDown={onEmailKeyDownHandler}/>
                <InputBox ref={passwordRef} label='비밀번호*' type={passwordType} placeholder='비밀번호를 입력해주세요.' value={password} onChange={onPasswordChangeHandler} error={isPassowrdError} message={passwordErrorMessage} icon={passwordButtonIcon} onButtonClick={onPasswordButtonClickHandler} onKeyDown={onPasswordKeyDownHandler}/>
            ...
            {page === 2 && (
              <>
                ...
                <InputBox ref={telNumberRef} label='핸드폰 번호*' type='text' placeholder='핸드폰 번호를 입력해주세요' value={telNumber} onChange={onTelNumberChangeHandler} error={isTelNumberError} message={telNumberErrorMessage} onKeyDown={onNicknameKeyDownHandler} />
